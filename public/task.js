...
document.addEventListener("DOMContentLoaded", function () {
  ...
  window.TaskApp.newReminderDrawer =
    document.getElementById("newReminderDrawer");
  ...
  window.TaskApp.drawerHeaderTitle =
  ...
  window.TaskApp.timePickerCancelBtn =
    document.getElementById("time-picker-cancel");
  window.TaskApp.timePickerSaveBtn =
    document.getElementById("time-picker-save");
  window.TaskApp.dialogOverlay = document.getElementById(
    "custom-dialog-overlay",
  );
  window.TaskApp.dialogMessage = document.getElementById(
    "custom-dialog-message",
  );
  window.TaskApp.dialogActions = document.getElementById(
  ...
  window.TaskApp.flowTimerPickerOverlay = document.getElementById(
    "flow-timer-picker-overlay",
  );
  window.TaskApp.flowTimerHoursInput = document.getElementById(
    "flow-timer-hours-input",
  );
  window.TaskApp.flowTimerMinutesInput = document.getElementById(
    "flow-timer-minutes-input",
  );
  window.TaskApp.flowTimerSecondsInput = document.getElementById(
    "flow-timer-seconds-input",
  );
  window.TaskApp.flowTimerCancelBtn =
    document.getElementById("flow-timer-cancel");
  ...
  window.TaskApp.openDrawer = function () {
    if (window.TaskApp.newReminderDrawer) {
      ...
      if (window.TaskApp.editingTaskId === null) {
        const selectorSpan =
          window.TaskApp.prioritySelector?.querySelector("span");
        if (selectorSpan) {
  ...
  window.TaskApp.closeDrawer = function () {
    if (window.TaskApp.newReminderDrawer) {
      ...
      if (window.TaskApp.drawerHeaderTitle)
        window.TaskApp.drawerHeaderTitle.innerHTML = "New Reminder";
  ...
  window.TaskApp.openDrawerForEdit = function (task) {
    ...
    if (window.TaskApp.activityInput)
      window.TaskApp.activityInput.value = task.title || "";
    if (window.TaskApp.timeInput) {
      ...
      if (task.endTimeMillis && task.endTimeMillis > 0) {
        window.TaskApp.timeInput.setAttribute(
          "data-end-millis",
          task.endTimeMillis.toString(),
        );
      } else {
        window.TaskApp.timeInput.removeAttribute("data-end-millis");
      }
  ...
  window.TaskApp.deleteTask = async function (taskId, user, reloadFn) {
    ...
    window.showCustomDialog("Are you sure you want to delete this reminder?", [
      { text: "Cancel", action: () => {}, isPrimary: false },
      {
        ...
        action: async () => {
          try {
            ...
            const taskToDelete = window.TaskApp.tasksData.find(
              (t) => t.id === taskId,
            );
            ...
            window.showCustomDialog("Reminder deleted successfully!", [
              {
                text: "OK",
                action: async () => {
            ...
            window.showCustomDialog("Failed to delete task.", [
...
// Handle Tombol Flow Timer (NEW)
      ...
      try {
        ...
        if (newDoneStatus) {
          try {
            ...
            initialStreakState.currentStreak =
              initialStreakState.currentStreak || 0;
          } catch (e) {
    ...
    for (const task of tasks) {
      ...
      if (taskDate < now) {
        try {
          ...
          window.TaskApp.tasksData = window.TaskApp.tasksData.filter(
            (t) => t.id !== task.id,
          );
        } catch (err) {
  ...
  function findAndActivateDateCard(dateString) {
    ...
    const card = calendarContainer.querySelector(
    ...
    if (card) {
      ...
      console.error(
        `Date card for ${dateString} not found in the displayed range.`,
      );
      const dateParts = dateString.split("-");
      const newActiveDate = new Date(
        dateParts[0],
  ...
  function generateCalendar(startDate, numberOfDays, tasks) {
    ...
    dates.forEach((date, index) => {
      ...
      const monthText = date.toLocaleDateString(ENGLISH_LOCALE, {
        month: "short",
      });
      const dayText = date.toLocaleDateString(ENGLISH_LOCALE, {
        weekday: "short",
      });
  ...
  function displayTasksForActiveDate(date) {
    ...
    const tasksForDate = window.TaskApp.tasksData.filter(
    ...
    if (tasksForDate.length === 0) {
      ...
      taskListContainer.innerHTML =
        '<div class="empty-task-message"><p>No reminders scheduled for this date.</p></div>';
    } else {
  ...
  function updateMonthYearDisplay(displayElement, date) {
    if (displayElement) {
      const monthYear = date.toLocaleDateString(ENGLISH_LOCALE, {
        year: "numeric",
        month: "long",
      });
  ...
  function handleScroll() {
    ...
    cards.forEach((card) => {
      const cardCenter =
        card.offsetLeft + card.offsetWidth / 2 - calendarContainer.scrollLeft;
    ...
    if (timePickerSaveBtn)
      timePickerSaveBtn.addEventListener("click", () => {
        ...
        if (endMillis <= startMillis) {
          ...
          if (window.TaskApp.dateInputEdit) {
            window.TaskApp.dateInputEdit.value = formatDate(
              window.TaskApp.activeDate,
            );
          }
        }
  ...
  function updateFlowTimerLinkText() {
    if (window.TaskApp.flowTimerLink) {
      const durationText = window.TaskApp.formatDurationToString(
        window.TaskApp.flowDurationMillis,
      );
  ...
  if (flowTimerPickerOverlay) {
    ...
    if (flowTimerSaveBtn)
      flowTimerSaveBtn.addEventListener("click", () => {
        if (
          !flowTimerHoursInput ||
          !flowTimerMinutesInput ||
          !flowTimerSecondsInput
        )
          return;
  ...
  // ====================================================
  window.TaskApp.prioritySelector =
    document.querySelector(".priority-selector");
  ...
  const priorityFormSection =
    window.TaskApp.prioritySelector?.closest(".form-section");
  ...
  function updatePriorityDropdownSelection(selectedLevel) {
    const priorityDropdown =
      window.TaskApp.priorityWrapper.querySelector(".priority-dropdown");
  ...
  function createPriorityDropdown() {
    if (!window.TaskApp.priorityWrapper || !window.TaskApp.prioritySelector)
      return;

    const currentLevel =
      window.TaskApp.prioritySelector.querySelector("span").textContent;
  ...
  document.addEventListener("click", function (e) {
    if (
      window.TaskApp.priorityWrapper &&
      window.TaskApp.priorityWrapper.classList.contains("open") &&
  ...
  const taskPageFormSubmitHandler = async function (event, user) {
    ...
    if (activity && dateToUse) {
      ...
      const endMillisAttr = timeInput
        ? timeInput.getAttribute("data-end-millis")
        : null;
      ...
      if (isTimeRangeSet) {
        ...
        if (inputTimeText.length === 0 && flowDurationMillis > 0) {
          const durationText =
            window.TaskApp.formatDurationToString(flowDurationMillis);
      ...
      try {
        ...
        if (finalStatus === "missed")
          successMessage = "Task moved to Missed Tasks!";
        else if (
        ...
        window.showCustomDialog(successMessage, [
          {
            ...
            action: () => {
              ...
              if (window.TaskApp.dateInputEdit)
                window.TaskApp.dateInputEdit.value = dateToUse;
              const selectorSpan =
                window.TaskApp.prioritySelector?.querySelector("span");
            ...
            action: async () => {
              ...
              if (window.location.pathname.includes("task.html")) {
                ...
                if (window.TaskApp.editMode === "restore") {
                  if (typeof renderDeletedTasksInDrawer === "function")
        ...
        console.error(
          `Error ${window.TaskApp.editingTaskId ? "updating" : "adding"} task:`,
          err,
        );
        window.showCustomDialog(`Failed to save task.`, [
          { text: "OK", action: () => {}, isPrimary: true },
        ]);
      }
    } else {
      window.showCustomDialog("Activity and Date are required!", [
        { text: "OK", action: () => {}, isPrimary: true },
      ]);
    }
  };
  ...
  window.TaskApp.startNewReminderFlow = function (dateToPreselect) {
    ...
    const activeDateForNew =
      dateToPreselect instanceof Date && !isNaN(dateToPreselect)
        ? dateToPreselect
        : new Date();
    ...
    if (window.TaskApp.dateInputEdit) {
      ...
      window.TaskApp.dateInputEdit.value =
        window.TaskApp.formatDate(activeDateForNew);
      window.TaskApp.dateInputEdit.removeAttribute("min");
    }
  ...
  firebase.auth().onAuthStateChanged((user) => {
    ...
    if (
      window.location.pathname.includes("task.html") ||
      window.location.pathname.includes("calendar.html")
    ) {
      loadTasksAndRenderCalendar(user, new Date("2025-01-01"), 365).then(() => {
    ...
    if (window.TaskApp.reminderForm) {
      ...
      window.TaskApp.reminderForm.addEventListener("submit", (event) =>
        taskPageFormSubmitHandler(event, user),
      );
    }
  });
