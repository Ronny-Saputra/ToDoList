...
function updateTaskCountsUI(stats) {
  ...
  if (completedBox)
    completedBox.textContent = `Completed Tasks (${stats.completed})`;
    ...
    if (dateObj && !isNaN(dateObj.getTime())) {
      ...
      if (fallbackDate) {
        ...
        if (dateParts.length === 3) {
          const dateObjFallback = new Date(
            dateParts[0],
            dateParts[1] - 1,
            dateParts[2],
          );
          ...
          if (!isNaN(dateObjFallback.getTime())) {
            ...
            dateStr =
              dateObjFallback.toLocaleDateString("en-US", {
                weekday: "long",
                year: "numeric",
                month: "long",
                day: "numeric",
...
function attachDrawerListeners() {
  ...
  const completedDrawerContent = document.getElementById(
    "completedDrawerContent",
  );
  const closeCompletedDrawer = document.getElementById("closeCompletedDrawer");
  const completedDrawerOverlay = document.getElementById(
    "completedDrawerOverlay",
  );
  ...
  const taskEditDrawerOverlay = document.getElementById(
    "taskEditDrawerOverlay",
  );
  ...
  const taskPrioritySelector = document.getElementById(
  ...
  document.addEventListener("click", function (e) {
    if (
      taskPriorityWrapper &&
      taskPriorityWrapper.classList.contains("open") &&
  ...
  if (taskEditForm) {
    taskEditForm.addEventListener("submit", async (e) => {
      ...
      const priority =
        taskPrioritySelector?.querySelector("span").textContent || "None";
      ...
      try {
        ...
        let successMessage =
          finalStatus === "missed"
            ? "Task moved to Missed Tasks!"
            : "Task successfully restored!";
