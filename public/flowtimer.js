...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.showCustomDialog = function(message, buttons) {
        ...
        if (!dialogOverlay || !dialogMessage || !dialogActions) {
            console.error("Custom dialog elements not found in the DOM. Cannot display complex dialog.");
            alert(message);
            return;
        }
    ...
    function showDurationPicker(onSave) {
        ...
        if (!pickerOverlay) {
             // ⬅️ TRANSLATION HERE
             window.showCustomDialog("Failed to load duration picker. Returning to task list.", [{ text: 'OK', action: () => window.location.href = '../pages/task.html', isPrimary: true }]);
            return;
        }
    ...
    function showCompletionDialog() {
        // ⬅️ TRANSLATION HERE
        window.showCustomDialog(
            "Time's up! Are you done with your task?",
            [
                { 
    ...
    function updateTimer() {
        if (timeLeft <= 0) {
            ...
            alarmAudio.play().then(() => {
                ...
                console.warn("Audio play blocked or failed. User interaction is required.", error);
            });
