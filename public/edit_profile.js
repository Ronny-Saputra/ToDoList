...
async function loadProfileData() {
  try {
    ...
    if (data.gender && selectedGenderSpan) {
      ...
      document
        .querySelectorAll(".gender-option i")
        .forEach((icon) => icon.classList.add("hidden"));
      const activeOption = document.querySelector(
        `.gender-option[data-value="${data.gender}"] i`,
      );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (saveBtn) {
    saveBtn.addEventListener("click", async (e) => {
      ...
      const gender = selectedGenderSpan
        ? selectedGenderSpan.textContent
        : "Male";
      ...
      try {
        ...
        window.showCustomDialog(
          "Failed to save profile. Check your internet connection.",
        );
      }
    });
  }
